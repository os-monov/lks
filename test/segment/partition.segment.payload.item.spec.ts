...
describe('PartitionSegmentPayloadItem', () => {
  ...
  describe('toBuffer', () => {
    it('should serialize to buffer correctly', () => {
      ...
      const expectedLength =
        4 +
        Buffer.byteLength(validKey, 'utf8') +
        4 +
        Buffer.byteLength(validValue, 'utf8');
      ...
      // Check key content
      expect(
        buffer.toString('utf8', 4, 4 + Buffer.byteLength(validKey, 'utf8')),
      ).toBe(validKey);

      // Check value length
      const keyByteLength = Buffer.byteLength(validKey, 'utf8');
      expect(buffer.readUInt32BE(4 + keyByteLength)).toBe(
        Buffer.byteLength(validValue, 'utf8'),
      );
      ...
      expect(
        buffer.toString(
          'utf8',
          8 + keyByteLength,
          8 + keyByteLength + Buffer.byteLength(validValue, 'utf8'),
        ),
      ).toBe(validValue);
    });
  });
  ...
  describe('from', () => {
    it('should deserialize from buffer correctly', () => {
      const originalItem = new PartitionSegmentPayloadItem(
        validKey,
        validValue,
      );
