...
describe('PartitionSegmentHeader', () => {
    ...
    describe('constructor', () => {
        it('should create a header with provided values', () => {
            const header = new PartitionSegmentHeader(partitionId, offset, payloadLength);
    ...
    describe('round-trip', () => {
        it('should maintain data integrity after serialization and deserialization', () => {
            const originalHeader = new PartitionSegmentHeader(partitionId, offset, payloadLength);
            const buffer = originalHeader.toBuffer();
            const roundTripHeader = PartitionSegmentHeader.from(buffer);

            expect(roundTripHeader.getPartitionId()).toBe(originalHeader.getPartitionId());
            expect(roundTripHeader.getOffset()).toBe(originalHeader.getOffset());
            expect(roundTripHeader.getPayloadLength()).toBe(originalHeader.getPayloadLength());
        });
    });
    ...
    describe('edge cases', () => {
        it('should handle maximum values', () => {
            ...
            const header = new PartitionSegmentHeader(maxPartitionId, maxOffset, maxPayloadLength);
            const buffer = header.toBuffer();
            const roundTripHeader = PartitionSegmentHeader.from(buffer);
