...
export class RecordLogManager implements OnModuleInit {
    ...
    async onModuleInit() {
        for (let logIndex = 0; logIndex < this.logCount; logIndex++) {
            ...
            console.log(
                `Initializing RecordLog ${logIndex} with ${cursors.length} partitions.`,
            );
            console.log(
                `Partitions: ${cursors.map((c) => `${c.partitionId}:${c.offset}`).join(', ')}`,
            );
            this.logs.push(
                new RecordLog(this.controlPlaneService, logIndex, 0, cursors),
            );
            console.log(
                `RecordLogManager: Initialized RecordLog ${logIndex} with ${cursors.length} partitions`,
            );
        }
    }
